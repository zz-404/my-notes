...
$$
\begin{aligned}
\frac{\partial J}{\partial w_j} &= \frac{1}{m}\sum_{i=1}^m (h^{(i)}-y^{(i)})x_j^{(i)} + \frac{\lambda}{m}w_j \\
...
### 在代价函数中添加正则项
- 正则项
    λ/m * sum_{j=1}^{n} w_j^2
- λ是正则化系数，过大时会欠拟合，过小则会过拟合

#### 线性回归正则化
$$
J(\mathbf{w}, b) = \frac{1}{2m}\sum_{i=1}^m (\mathbf{w}^T\mathbf{x}^{(i)} + b - y^{(i)})^2 + \frac{\lambda}{2m}\sum_{j=1}^n w_j^2
$$
梯度下降
$$
\begin{aligned}
...
#### 逻辑回归正则化
$$
J(\mathbf{w}, b) = -\frac{1}{m}\sum_{i=1}^m \left[y^{(i)}\log(h^{(i)}) + (1-y^{(i)})\log(1-h^{(i)})\right] + \frac{\lambda}{2m}\sum_{j=1}^n w_j^2
$$
梯度下降
$$
\begin{aligned}
