...
### 使用多个决策树
单个决策树的构建通常对训练集中样本的微小变化感到敏感，采用构建多个决策树并投票的方式可以获得更加公平的结果  
**构建树集成**
```python
for i in B(训练的树的个数):
   用放回抽样法获得一个新训练集
   用新训练集训练得到一个决策树
```
*一味增加树的个数到后面会缩小增益（构建了太多相同的树且降低了计算速度）*  
随机森林算法的优化  
对于具有大量特征的问题，假设有n个特征，每次随机挑选k(<n)个特征作子集，再从该子集中选出信息增益最大的作为分裂特征，k通常为sqrt(n)     
- 这样可以增加树的种类，降低关联性，提高泛化能力
- 提高计算效率，加快模型训练速度
- 减轻过拟合，k使得决策树构建更为随机，减轻了过拟合风险

### XGBoost
在构建新训练集时以更高的概率选中那些在之前所构建的B-1个树中表现不佳的样本（刻意训练）  
该模型甚至为不同的训练样本分配不同的权重  
```python
from xgboost import XGBClassifier
```

### 何时使用决策树和树集成
决策树适合用于结构化数据（表格等），速度快
神经网络适用于非结构化数据（图片，语音等），速度慢，可以迁移学习
